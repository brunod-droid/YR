  ...
  for (const file of Array.from(fileList || [])) {
    ...
    if (name.toLowerCase().endsWith(".zip")) {
      ...
      for (const [innerName, entry] of Object.entries(zip.files)) {
        ...
        if (!/\.(csv|xlsx)$/i.test(innerName)) continue;
        const blob = await entry.async("blob");
        expanded.push(new File([blob], innerName.split("/").pop(), { type: blob.type }));
      }
    } else {
      expanded.push(file);
    }
...
export default function MonthlyUploadPage() {
  ...
  async function handleFiles(files) {
    ...
    try {
      const report = await buildReportForMonth(monthValue);
      const uploadDetails = [];
      const expandedFiles = await expandUploadFiles(fileList);
      for (const file of expandedFiles) {
        ...
        const rawRows = await parseDataFile(file);
  ...
  return <main style={pageStyle}>
    <ReportingNav />
    <h1 style={{ fontSize: 42, fontWeight: 900 }}>Upload Monthly CSV / XLSX / ZIP files</h1>
    ...
    <div onDragEnter={(e) => { e.preventDefault(); setIsDragging(true); }} onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }} onDragLeave={(e) => { e.preventDefault(); setIsDragging(false); }} onDrop={onDrop} style={{ ...cardStyle, marginTop: 20, minHeight: 190, border: isDragging ? "4px dashed #7c2d12" : "4px dashed #94a3b8", background: isDragging ? "#ffedd5" : "#ffffff", textAlign: "center", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}><div style={{ fontSize: 30, fontWeight: 950 }}>Step 3 - Drag and drop Monthly CSV / XLSX / ZIP files here</div><div style={{ marginTop: 10, color: "#64748b" }}>Use monthly exports. ZIP files are supported and will be unpacked automatically.</div><input type="file" accept=".csv,.xlsx,.xls,.zip,text/csv" multiple onChange={(e) => handleFiles(e.target.files)} style={{ marginTop: 22, fontSize: 16 }} /></div>
