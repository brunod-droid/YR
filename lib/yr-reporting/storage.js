import { SHARED_REPORTS } from "./sharedReports";
...
export function getReports() {
  const shared = Array.isArray(SHARED_REPORTS) ? SHARED_REPORTS : [];
  if (!isBrowser()) return shared;
  return mergeReports(getLocalReportsOnly(), shared);
}
...
export function upsertReport(report) {
  ...
  if (index >= 0) {
    ...
    reports.push({ ...next, createdAt: new Date().toISOString() });
  }
