...
export function ReportingNav() {
  ...
  return <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 22 }}>{items.map(([label, href]) => <Link key={href} href={href} style={{ background: href === "/" ? "#7c3aed" : href.includes("finance") ? "#991b1b" : href.includes("monthly-upload") ? "#7c2d12" : href.includes("data") ? "#15803d" : "#0f172a", color: "#fff", borderRadius: 12, padding: "10px 14px", textDecoration: "none", fontWeight: href === "/" || href.includes("finance") || href.includes("data") || href.includes("monthly-upload") ? 900 : 700 }}>{label}</Link>)}</div>;
}
