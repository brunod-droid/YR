...
export function ReportingNav() {
  const items = [
    ...
    ["Publish data", "/yves-rocher-reporting/publish"],
    ...
    <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 22 }}>
      {items.map(([label, href]) => (
        <Link
          ...
          href={href}
          style={{
            background: href === "/" ? "#7c3aed" : href.includes("finance") ? "#991b1b" : href.includes("publish") ? "#b45309" : href.includes("data") ? "#15803d" : "#0f172a",
            color: "#fff",
            borderRadius: 12,
            padding: "10px 14px",
            textDecoration: "none",
            fontWeight: href === "/" || href.includes("finance") || href.includes("data") || href.includes("publish") ? 900 : 700
          }}
        >
          {label}
        </Link>
