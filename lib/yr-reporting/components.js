...
export function ReportingNav() {
  ...
  return <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 22 }}>{items.map(([label, href]) => <Link key={href} href={href} style={{ background: href === "/" ? "#7c3aed" : href.includes("h1") ? "#15803d" : href.includes("kpi-dictionary") ? "#475569" : href.includes("finance") ? "#991b1b" : href.includes("monthly-upload") ? "#7c2d12" : href.includes("data") ? "#15803d" : href.includes("developments") ? "#0369a1" : href.includes("future-plans") ? "#4338ca" : href.includes("risks") ? "#b45309" : "#0f172a", color: "#fff", borderRadius: 12, padding: "10px 14px", textDecoration: "none", fontWeight: href === "/" || href.includes("finance") || href.includes("data") || href.includes("monthly-upload") || href.includes("developments") || href.includes("future-plans") || href.includes("risks") ? 900 : 700 }}>{label}</Link>)}</div>;
}
