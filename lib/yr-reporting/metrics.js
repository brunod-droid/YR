...
function ticketCreatedAt(row = {}) {
  const raw = row["Created at"] || row["Creation date"] || row["Created At"] || row.created_at || row.createdAt || "";
  if (!raw) return null;
  const date = new Date(String(raw).trim().replace(" ", "T"));
  return Number.isNaN(date.getTime()) ? null : date;
}

function reportStartDate(report = {}) {
  const explicit = report.startDate || report.start || report.weekStart || report.periodStart || "";
  const weekStart = String(report.week || "").split("_")[0] || "";
  const raw = explicit || weekStart;
  if (!raw) return null;
  const date = new Date(`${String(raw).slice(0, 10)}T00:00:00`);
  return Number.isNaN(date.getTime()) ? null : date;
}

function filterTicketsFromReportStart(tickets = [], report = {}) {
  const start = reportStartDate(report);
  if (!start) return tickets;
  return tickets.filter((ticket) => {
    const created = ticketCreatedAt(ticket);
    // Keep rows with an unreadable date so we do not silently drop valid current-period data.
    return !created || created >= start;
  });
}

function buildAgentDrilldown(tickets = [], agentMetrics = [], frtTickets = tickets, allowAgentMetricFrt = true) {
  const map = {};
  ...
  // SLA/FRT is calculated only from tickets created on or after the report start date.
  frtTickets.forEach((ticket) => {
    const assignee = ticketAssignee(ticket);
    if (!assignee || isAverageName(assignee)) return;
    const item = ensure(assignee);
  ...
  cleanAgentMetrics.forEach((row) => {
    ...
    if (allowAgentMetricFrt && frt > 0 && item.frtCount === 0) { item.frtTotal = frt; item.frtCount = 1; }
...
function weightedSla(agents = [], filterFn = () => true) {
  const valid = agents.filter((a) => filterFn(a) && a.avgFirstResponseHours > 0 && a.frtCount > 0);
  const responses = valid.reduce((sum, a) => sum + a.frtCount, 0);
  if (!responses) return 0;
  return valid.reduce((sum, a) => sum + a.avgFirstResponseHours * a.frtCount, 0) / responses;
}

function countOrders(orders = []) {
  const explicit = sumBy(orders, [/^paidorders$/, /^orders$/, /paid orders/, /order count/, /total orders/]);
...
function orderSummary(orders = []) {
  const row = orders[0] || {};
  return {
    totalOrders: toNumber(row.TotalOrders || row["Total Orders"] || row.totalOrders || 0) || orders.length,
    paidOrders: toNumber(row.PaidOrders || row.Orders || row["Paid Orders"] || row.orders || 0) || countOrders(orders),
    cancelledOrders: toNumber(row.CancelledOrders || row["Cancelled Orders"] || row.cancelledOrders || 0),
    refundedOrders: toNumber(row.RefundedOrders || row["Refunded Orders"] || row.refundedOrders || 0),
    fraudOrders: toNumber(row.FraudOrders || row["Fraud Orders"] || row.fraudOrders || 0),
    grossRevenue: toNumber(row.GrossRevenue || row["Gross Revenue"] || row.Revenue || row.Total || 0),
    refundAmount: toNumber(row.RefundAmount || row["Refund Amount"] || row.Refunded || 0)
  };
}
...
export function calculateWeeklyMetrics(report, settings) {
  ...
  const slaTickets = filterTicketsFromReportStart(tickets, report);
  const hasReportStart = Boolean(reportStartDate(report));
  ...
  const ordersInfo = orderSummary(orders);
  const ordersCount = ordersInfo.paidOrders;
  ...
  const agentDrilldown = buildAgentDrilldown(tickets, agents, slaTickets, !hasReportStart);
  ...
  return {
    ...
    backlog,
    ordersCount,
    orderSummary: ordersInfo,
    totalOrders: ordersInfo.totalOrders,
    paidOrders: ordersInfo.paidOrders,
    cancelledOrders: ordersInfo.cancelledOrders,
    refundedOrders: ordersInfo.refundedOrders,
    fraudOrders: ordersInfo.fraudOrders,
    grossRevenue: ordersInfo.grossRevenue,
    refundAmount: ordersInfo.refundAmount,
    refundRate: ordersInfo.paidOrders ? ordersInfo.refundedOrders / ordersInfo.paidOrders : 0,
    cancellationRate: ordersInfo.totalOrders ? ordersInfo.cancelledOrders / ordersInfo.totalOrders : 0,
    fraudRate: ordersInfo.totalOrders ? ordersInfo.fraudOrders / ordersInfo.totalOrders : 0,
    unpaidOrRemovedOrders: Math.max((ordersInfo.totalOrders || 0) - (ordersInfo.paidOrders || 0), 0),
    ...
    debug: {
      ...
      slaTicketsRows: slaTickets.length,
      excludedPrePeriodTickets: Math.max(tickets.length - slaTickets.length, 0),
      reportStartDate: reportStartDate(report)?.toISOString?.().slice(0, 10) || "",
