...
function countOrders(orders = []) {
  return getOrderMetrics(orders).paidOrders;
}

export function getOrderMetrics(orders = []) {
  const paidOrders = sumBy(orders, [/^paid orders$/, /^orders$/, /order count/]);
  const totalOrders = sumBy(orders, [/^total orders$/]) || (paidOrders ? paidOrders : orders.length);
  const cancelledOrders = sumBy(orders, [/^cancelled orders$/]);
  const refundedOrders = sumBy(orders, [/^refunded orders$/]);
  const partiallyRefundedOrders = sumBy(orders, [/^partially refunded orders$/]);
  const fraudOrders = sumBy(orders, [/^fraud orders$/]);
  const revenue = sumBy(orders, [/^revenue$/, /^total sales$/, /^net sales$/]);
  const refundedAmount = sumBy(orders, [/^refunded amount$/, /^refund amount$/]);

  if (paidOrders > 0 || totalOrders > 0) {
    return { paidOrders, totalOrders, cancelledOrders, refundedOrders, partiallyRefundedOrders, fraudOrders, revenue, refundedAmount };
  }

  const paidRows = orders.filter(isPaidOrder);
  return { paidOrders: paidRows.length, totalOrders: orders.length, cancelledOrders: 0, refundedOrders: 0, partiallyRefundedOrders: 0, fraudOrders: 0, revenue: 0, refundedAmount: 0 };
}
...
export function calculateWeeklyMetrics(report, settings) {
  ...
  const ordersCount = orderMetrics.paidOrders;
  ...
  return {
    ...
    backlog,
    ordersCount,
    paidOrders: orderMetrics.paidOrders,
    totalOrders: orderMetrics.totalOrders,
    cancelledOrders: orderMetrics.cancelledOrders,
    refundedOrders: orderMetrics.refundedOrders,
    partiallyRefundedOrders: orderMetrics.partiallyRefundedOrders,
    fraudOrders: orderMetrics.fraudOrders,
    revenue: orderMetrics.revenue,
    refundedAmount: orderMetrics.refundedAmount,
    refundRate: orderMetrics.paidOrders ? orderMetrics.refundedOrders / orderMetrics.paidOrders : 0,
    cancellationRate: orderMetrics.totalOrders ? orderMetrics.cancelledOrders / orderMetrics.totalOrders : 0,
    fraudRate: orderMetrics.totalOrders ? orderMetrics.fraudOrders / orderMetrics.totalOrders : 0,
    unpaidOrRemovedOrders: Math.max((orderMetrics.totalOrders || 0) - (orderMetrics.paidOrders || 0), 0),
